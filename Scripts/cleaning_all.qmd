...
#Load in the files
files <- list.files(path="/Users/idahelenedencker/Desktop/STANDBY/Facebook_Field_2023-kopi/data/preprocessed/second_round_json", pattern="*.json", full.names=TRUE, recursive=FALSE)
...
data_frame_all$file_name <- sub("/Users/idahelenedencker/Desktop/STANDBY/Facebook_Field_2023-kopi/data/preprocessed/second_round_json/", "" , data_frame_all$file_name)
...
data_frame_all <- data_frame_all[, c(2, 3, 27, 15, 17, 16, 18, 28, 31, 30, 5, 6, 12, 4, 7, 29, 8, 9, 11, 1, 10, 14, 13, 26, 19, 20, 21, 22, 23, 24, 25 )]
```



```{r}
#make a compliance column
...
#/Users/idahelenedencker/Desktop/STANDBY/

write.csv(data_frame_treatment, file = "/Users/idahelenedencker/Desktop/STANDBY/treatment_data.csv", row.names = FALSE)

print(data_frame_treatment$file_name) #print all the 203 filenames 

length(unique(data_frame_treatment$name_abbreviation)) #only 19 politicians, obs: no PoBa that are treatment so number is correct 


#based on searching: 51 contains 
sum(grepl("SIG FRA, ANMELD, STØT OP", data_frame_all$text))
...
write.csv(data_frame_all, "/Users/idahelenedencker/Desktop/STANDBY/all_files_preliminary.csv", row.names=FALSE)
```
