...
```{r}
#this code will run but gives a data_frame_all of 23832 (rows) x 3668 (columns)
#problem being that column names need to be the same for it to bind dataframe the right way
...
#figure out how to deal with problematic files
```{r}
#first: load in only the problematic file to see how it looks before combining i.e. data_frame 
  #
# then load in 35+1 file
...
data_frame_all$file_name <- sub("/Users/idahelenedencker/Desktop/STANDBY/Facebook_Field_2023-kopi/data/test_data_35_files/", "" , data_frame_all$file_name)
...
for (id_value in unique(data_frame_all$ID)) {
  ...
  for (i in 1:nrow(subset_df)) {
    ...
    subset_df$n_total_reactions_com[i] <- sum(subset_df[i, 35:41], na.rm = TRUE)
...
for (row in 1:nrow(data_frame_all)) {
  ...
  if (data_frame_all$name_abbreviation[row] == 'BüSi' &
    ...
    else if (data_frame_all$name_abbreviation[row] == 'HüAr' & 
    ...
    else if (data_frame_all$name_abbreviation[row] == 'MeAy' & 
